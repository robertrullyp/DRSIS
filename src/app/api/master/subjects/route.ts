...
import { subjectCreateSchema } from "@/lib/schemas/master";
...
export async function GET(req: NextRequest) {
  ...
  const [items, total] = await Promise.all([
    prisma.subject.findMany({ where, orderBy: { name: "asc" }, include: { grade: true, curriculum: true }, skip: (page - 1) * pageSize, take: pageSize }),

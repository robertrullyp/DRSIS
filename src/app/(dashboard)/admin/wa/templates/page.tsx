...
type Tpl = { id: string; key: string; content: string };

export default function WaTemplatesPage() {
  const qc = useQueryClient();
  const { data } = useQuery<{ items: Tpl[] }>({ queryKey: ["wa-templates"], queryFn: async () => (await fetch("/api/admin/wa/templates")).json() });
  ...
  const create = useMutation({
    mutationFn: async () => {
      const res = await fetch("/api/admin/wa/templates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ key, content }) });
      if (!res.ok) throw new Error("Create failed");
    },
    onSuccess: () => { setKey(""); setContent(""); qc.invalidateQueries({ queryKey: ["wa-templates"] }); },
  });
  const remove = useMutation({
    mutationFn: async (id: string) => { const res = await fetch(`/api/admin/wa/templates/${id}`, { method: "DELETE" }); if (!res.ok) throw new Error("Delete failed"); },
  ...
  const sendTest = useMutation({
    mutationFn: async () => { const res = await fetch("/api/admin/wa/send-test", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ to: testTo, key: testKey || key, payload: testPayload }) }); if (!res.ok) throw new Error("Send test failed"); },
  });

  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold">Admin: WA Templates</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); if (!key || !content) return; create.mutate(); }} className="grid md:grid-cols-2 gap-3">
        <div>
      ...
      <div className="rounded-xl p-4 glass-card space-y-2">
        ...
        <div className="grid md:grid-cols-3 gap-3 items-end">
          <div>
            <label className="block text-xs text-muted-foreground mb-1">Nomor WA (to)</label>
            <input className="border rounded px-3 py-2 w-full" placeholder="628xxxx" value={testTo} onChange={(e) => setTestTo(e.target.value)} />
          </div>
          <div>
      ...
      <table className="w-full text-sm border">
        ...
        <tbody>
          ...
          {(data?.items?.length ?? 0) === 0 && (
            <tr><td className="p-2" colSpan={3}>Belum ada template.</td></tr>
          )}
        </tbody>
      </table>
    </div>

...
export default function AdminCmsEventsPage() {
  ...
  const currentPageSize = eventsQuery.data?.pageSize ?? pageSize;
  ...
  return (
    <div className="space-y-4">
      ...
      {eventsQuery.isLoading ? (
        ...
        <DataTable minWidthClassName="min-w-[1280px]">
          <DataTableHead>
            <DataTableRow>
              <DataTableHeaderCell>
              ...
              <DataTableHeaderCell>Lokasi</DataTableHeaderCell>
              <DataTableHeaderCell>Mulai</DataTableHeaderCell>
              <DataTableHeaderCell>Selesai</DataTableHeaderCell>
          ...
          <tbody>
            {rows.map((item) => (
              <DataTableRow key={item.id}>
                ...
                <DataTableCell>{item.location || "-"}</DataTableCell>
                <DataTableCell>{formatDateTime(item.startAt)}</DataTableCell>
                <DataTableCell>{formatDateTime(item.endAt)}</DataTableCell>
                <DataTableCell>{item.status}</DataTableCell>
                <DataTableCell>{formatDateTime(item.publishedAt)}</DataTableCell>
                <DataTableCell>
            ...
            {rows.length === 0 ? <DataTableEmptyRow message="Belum ada event." colSpan={10} /> : null}
          </tbody>
        </DataTable>
      )}
    </div>

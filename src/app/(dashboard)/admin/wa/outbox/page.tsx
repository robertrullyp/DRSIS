...
type Row = { id: string; to: string; status: string; providerMsgId?: string | null; sentAt?: string | null; template?: { key: string } | null };

export default function WaOutboxPage() {
  const qc = useQueryClient();
  const [status, setStatus] = useState<string>("");
  const { data, isFetching } = useQuery<{ items: Row[] }>({ queryKey: ["wa-outbox", status], queryFn: async () => (await fetch(`/api/admin/wa/outbox${status ? `?status=${status}`: ""}`)).json() });

  const send = useMutation({
    mutationFn: async () => { const res = await fetch(`/api/admin/wa/outbox/send?limit=50`, { method: "POST" }); if (!res.ok) throw new Error("Send failed"); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["wa-outbox"] }),
  });
  const retry = useMutation({
    mutationFn: async (id: string) => { const res = await fetch(`/api/admin/wa/outbox/${id}/retry`, { method: "POST" }); if (!res.ok) throw new Error("Retry failed"); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["wa-outbox"] }),
  });
  const cancel = useMutation({
    mutationFn: async (id: string) => { const res = await fetch(`/api/admin/wa/outbox/${id}/cancel`, { method: "POST" }); if (!res.ok) throw new Error("Cancel failed"); },
    onSuccess: () => qc.invalidateQueries({ queryKey: ["wa-outbox"] }),
  });

  return (
    <div className="space-y-4">
      <h1 className="text-lg font-semibold">Admin: WA Outbox</h1>
